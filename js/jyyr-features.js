...
(function(){
  ...
  async function renderPayments(){
    ...
    const render=()=>{
      ...
      const pays=adminCache.payments.filter(p=>(!term||`${p.id} ${p.orders?.id||''} ${p.orders?.profiles?.username||''}`.toLowerCase().includes(term))&&(!f||p.status===f));
      document.getElementById('paymentFeatureList').innerHTML=pays.map(p=>`<div class="feature-user-card"><div><b>${esc(p.id)}</b><small>${esc(p.orders?.profiles?.username||'')} • ${esc(p.method)}</small></div><div><b>${rup(p.orders?.total)}</b><small>${esc(p.status)} • ${fmt(p.created_at)}</small></div><div class="admin-actions">${p.status==='pending'?`<button onclick="jyyrReviewPayment('${p.id}','verified')">${jyyrIcon('check')}</button><button onclick="jyyrReviewPayment('${p.id}','rejected')">${jyyrIcon('cancel')}</button>`:''}<button onclick="jyyrPaymentDetail('${p.id}')">${jyyrIcon('file')} Detail</button></div></div>`).join('')||'<div class="feature-empty">Tidak ada payment.</div>';
  ...
  async function renderSecurity(){
    ...
    document.getElementById('adminContent').innerHTML=`<div class="admin-section-head"><div><h3>${jyyrIcon('shield')} Security Admin</h3><small>Audit, 2FA, sessions, permissions.</small></div></div>
      <div class="dashboard-section"><h4>Permission</h4><p>Level: <b>${esc(p.admin_level==='super_admin'?'OWNER':(p.admin_level||'admin'))}</b></p><p>${p.admin_level==='super_admin'?'Owner terkunci permanen dan memiliki akses penuh. Owner tidak dapat diganti atau diturunkan.':'Permission akun ini diatur oleh Owner. Kamu tidak dapat menaikkan level sendiri.'}</p>
      <div class="feature-list">${adminCache.users.filter(x=>x.role==='admin').map(a=>`<div class="feature-row"><div><b>${esc(a.username)} <span class="admin-role-badge">• ${esc(a.admin_level==='super_admin'?'owner':(a.admin_level||'admin'))}</span></b><small>${esc(a.admin_level==='super_admin'?'Owner terkunci':(a.admin_level||'admin'))}</small></div>${p.admin_level==='super_admin' && a.admin_level!=='super_admin'?`<select class="input" onchange="jyyrSetAdminLevel('${a.id}',this.value)"><option value="admin" ${a.admin_level==='admin'?'selected':''}>Admin</option><option value="readonly" ${a.admin_level==='readonly'?'selected':''}>Read-only</option></select>`:''}</div>`).join('')}</div></div>
  ...
  window.jyyrSetOwnAdminLevel=async()=>{toast('Level akun hanya dapat diatur oleh Owner.','err');};
